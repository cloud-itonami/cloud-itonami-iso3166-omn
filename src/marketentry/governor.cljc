(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether an Oman-resident
  commercial registration is actually on file, whether a claimed
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Oman-resident-entity missing -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-om-entity?
                                       true` (Esnad supplier/tenderer
                                       registration is gated on an
                                       Invest Easy Commercial
                                       Registration -- see
                                       `marketentry.facts`),
                                       ...
                                       Grounded in the Ministry of
                                       Commerce, Industry and
                                       Investment Promotion (MOCIIP)
                                       Invest Easy Commercial
                                       Registration (CR) that Esnad
                                       supplier registration is built
    ...
    5. Corporate-number unverified -- for `:filing/submit`, when the
                                       ...
                                       `:cr-verified?` -- the Oman Tax
                                       Authority runs an actual
                                       independent CR/VATIN
                                       cross-check e-service
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CR/Invest Easy登録/Esnad登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- om-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-om-entity? true`, INDEPENDENTLY verify
  `:has-om-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-om-entity?` ground truth (Esnad supplier/tenderer
  registration is gated on an Invest Easy Commercial Registration; a
  pure-foreign-market engagement may not have completed it yet)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-om-entity? e))
                 (not (true? (:has-om-entity? e))))
        [{:rule :om-entity-missing
          :detail (str subject " はオマーンの商業登記(CR/Invest Easy)を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (om-entity-missing-violations request st)
