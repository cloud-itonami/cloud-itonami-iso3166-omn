  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems OM"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems OM" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest om-entity-missing-is-held-and-unoverridable
  (testing "missing Oman commercial-registration entity -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:om-entity-missing} (-> (store/ledger db) last :basis)))
